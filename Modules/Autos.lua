...
local function Init(AutosCollect, AutosQuest, AutosJack, AutosFavorit, AutosAppraise, AutoAppraise, AutoEnchant, Collect, AutosSection, AuraSection, AutoJetskiSection, FoodSection)
    ...
    pcall(function()
        ...
        local announcements = playerGui and playerGui:FindFirstChild("hud")
            and playerGui.hud:FindFirstChild("safezone")
            and playerGui.hud.safezone:FindFirstChild("announcements")
        if announcements then
            announcements.ChildAdded:Connect(function(child)
                task.wait(0.05)
                local main = child:FindFirstChild("Main")
                if main and main:IsA("TextLabel") and main.Text and string.find(main.Text:lower(), "appraise") then
